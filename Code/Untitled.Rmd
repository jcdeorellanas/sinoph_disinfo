...
### Data frame and table organizing the 2021 videos by date
videos_2021_wilpbo <- video_data |>
  filter(video_publish_date >= as_date("2021-01-01") & video_publish_date <= as_date("2021-12-31")) |> 
  mutate(channel_title = str_replace_all(channel_title,
                                         "Willax Televisión", "Willax")) |>
  select(-c(video_id, channel_id, video_description, video_dislike_count, video_tags),
         Channel = channel_title,
...
march_2021_videos <- video_data |>
  filter(video_publish_date >= as_date("2021-03-01") & video_publish_date <= as_date("2021-03-31")) |> 
  mutate(channel_title = str_replace_all(channel_title,
                                   "Willax Televisión", "Willax")) |>
  select(-c(video_id, channel_id, video_description, video_dislike_count, video_tags),
         Channel = channel_title,
