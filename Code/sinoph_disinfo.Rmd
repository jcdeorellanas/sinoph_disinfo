...
video_data <- rio::import("../data/willax_pbo_youtube_vids/df_sinoph_pbo_willax_disinfo_long.csv")
```
...
### Table organizing the 2020 - 2021 videos by date
```{r, echo=FALSE}

videos_wilpbo <- video_data |>
  filter(video_publish_date >= as_date("2020-10-01") & video_publish_date <= as_date("2021-12-31")) |> 
  ...
  arrange(Published, Aired)

```


```{r, echo=FALSE}
date_table_head <- head(videos_wilpbo, n=23) |>
  mutate(Published = as.factor(Published)) |>
  gt() |>
  tab_header(
    title = "Videos Oct.'20 - Dec. '21"
  )|>
  data_color(columns = c(Published, Views),
             direction = "row",
             target_columns = NULL,
             method = "auto",
             bins = 4,
...
date_table_head
```
### Table with first videos of 2021 about the COVID vaccine 
```{r, echo=FALSE}
vids_2021 <- videos_wilpbo |>
  filter(Published >= as_date("2021-01-01") & Published <= as_date("2021-12-31")) |>
  head(n=20) |>
  mutate(Published = as.factor(Published)) |>
  gt() |>
  tab_header(
    title = "Videos 2021 (first 20)
    )"
  )|>
  data_color(columns = c(Published, Views),
             direction = "row",
...
views_table_top <- views_df_sliced |>
  ...
  gt() |>
  tab_header(
    title = "Total of views per video - Head"
  )|>
  data_color(columns = c(Published, Views),
             direction = "column",
             target_columns = NULL,
...
```{r, echo=FALSE}
march_2021_videos <- videos_wilpbo |>
  filter(Published >= as_date("2021-03-01") & Published <= as_date("2021-03-31")) |> 
  arrange(Published, Aired)
...
march_2021_table <- march_2021_videos |>
  ...
  data_color(columns = everything(),
             rows = str_detect(Published, regex(
               "2021-03-06")),
             direction = "row",
             target_columns = NULL,
             method = "auto",
...
```{r, echo=FALSE}
stat_summ_df <- summary(videos_wilpbo) |>
...
stat_summ_df <- stat_summ_df |>
  rename(Videos = "Title") |>
  select(-c(Channel, Program, Aired))
...
```{r, echo=FALSE}
videos_wilpbo |> ggplot(aes(x = Published, y = Views, color = Program, group = Program)) + 
  geom_jitter() +
...
```{r, echo=FALSE}
videos_wilpbo |> ggplot(aes(x = Published, y = Comments, color = Program, group = Program)) + 
  geom_jitter() +
...
```{r, echo=FALSE}
videos_wilpbo |> ggplot(aes(x = Published, y = Likes, color = Program, group = Program)) + 
  geom_jitter() +
